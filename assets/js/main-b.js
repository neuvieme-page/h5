...
(function($,window) {
    ...
    /* New video wrapper */
    if (!Modernizr.touchevents) {
        if($('.custom-video-wrapper').length) {
    ...
    $('.slider').on('afterChange', function(event, slick, currentSlide){
        ...
        if ($('.slider .custom-video-wrapper').hasClass('slick-active')) {
            $('.custom-video-wrapper.slick-active').find('iframe[src*="vimeo.com"]').each(function () {
               player.api('play');
               player.api('setVolume',1);
            });
        } else {
